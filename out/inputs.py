'''
FORMAT:
*_ips = [
    [input: string, semantic: dict]
    .
    .
    .
    [...,...]
]
'''

arr_ips = [
    ['declare an array called x with type float of size 10',
     {'request': 'declare', 'construct': 'array',
      'name': 'x', 'type': 'float', 'size': (10, )}],
    ['declare an integer array x of size 10 by 20',
     {'request': 'declare', 'construct': 'array',
      'name': 'x', 'type': 'int', 'size': (10, 20)}],
    ['declare an array of type int called x of size 10',
     {'request': 'declare', 'construct': 'array',
      'name': 'x', 'type': 'int', 'size': (10, )}],
    ['declare an integer array of size 20 called x',
     {'request': 'declare', 'construct': 'array',
      'name': 'x', 'type': 'int', 'size': (20, )}],
    ['declare an array of size 20 of type int called x',
     {'request': 'declare', 'construct': 'array',
      'name': 'x', 'type': 'int', 'size': (20, )}],
    ['declare an array  called x of size 20 type int',
     {'request': 'declare', 'construct': 'array',
      'name': 'x', 'type': 'int', 'size': (20, )}],
    ['declare an integer type array called prog count',
     {'request': 'declare', 'construct': 'array',
      'name': 'prog_count', 'type': 'int'}],
]

cond_ips = [
    ['a less than 10',
     {'cond': ('<', ('name', 'a'), ('value', 10))}],
    ['program count greater than 20',
     {'cond': ('>', ('name', 'program_count'), ('value', 20))}],
    ['a greater than 2',
     {'cond': ('>', ('name', 'a'), ('value', 2))}],
    ['a is greater than 5 and a is less than 10',
     {'cond': (
        '&&',
        ('>', ('name', 'a'), ('value', 5)),
        ('<', ('name', 'a'), ('value', 10))
      )}],
    ['a is equal to 10',
     {'cond': ('==', ('name', 'a'), ('value', 10))}],
    ['a is not equal to 10',
     {'cond': ('!=', ('name', 'a'), ('value', 10))}],
    ['a equals 10',
     {'cond': ('==', ('name', 'a'), ('value', 10))}],
    ['a equal to 10',
     {'cond': ('==', ('name', 'a'), ('value', 10))}],
    ['a not equal to 10',
     {'cond': ('!=', ('name', 'a'), ('value', 10))}],
    ['a not greater than 5',
     {'cond': ('<=', ('name', 'a'), ('value', 5))}],
    ['i is less than test case',
     {'cond': ('<', ('name', 'i'), ('name', 'test_case'))}],
    ['a smaller than b',
     {'cond': ('<', ('name', 'a'), ('name', 'b'))}],
    ['x bigger than y',
     {'cond': ('>', ('name', 'x'), ('name', 'y'))}],
    ['a greater than equal to 20',
     {'cond': ('>=', ('name', 'a'), ('value', 20))}],
    ['b less than or equal to y',
     {'cond': ('<=', ('name', 'b'), ('name', 'y'))}],
    ['a less than 20 and b greater than x or c less than d',
     {'cond': (
        '&&',
        ('<', ('name', 'a'), ('value', 20)),
        ('||',
         ('>', ('name', 'b'), ('name', 'x')),
         ('<', ('name', 'c'), ('name', 'd')))
      )}],
    ['true', {'cond': ('value', 'true')}],
    ['false', {'cond': ('value', 'false')}],
    ['x plus 2 less than 10',
     {'cond': ('<', ('+', ('name', 'x'), ('value', 2)), ('value', 10))}],
]

exp_ips = [
    ['2 times 2',
     {'exp': ('*', ('value', 2), ('value', 2))}],
    ['2 times 2 plus 1',
     {'exp': ('*', ('value', 2), ('+', ('value', 2), ('value', 1)))}],
    ['2 by 2',
     {'exp': ('/', ('value', 2), ('value', 2))}],
    ['2 divided by 2',
     {'exp': ('/', ('value', 2), ('value', 2))}],
    ['x plus 10',
     {'exp': ('+', ('name', 'x'), ('value', 10))}],
    ['increment x',
     {'exp': ('++', ('name', 'x'))}],
    ['10 times 5 plus 1',
     {'exp': ('*', ('value', 10), ('+', ('value', 5), ('value', 1)))}],
    ['2 minus 1',
     {'exp': ('-', ('value', 2), ('value', 1))}],
    ['minus 1 minus 2',
     {'exp': ('-', ('-', ('value', 1), ('value', 2)))}],
    ['decrement y',
     {'exp': ('--', ('name', 'y'))}],
    ['x mod 10',
     {'exp': ('%', ('name', 'x'), ('value', 10))}],
    ['x modulo 10',
     {'exp': ('%', ('name', 'x'), ('value', 10))}],
    ['x by 10',
     {'exp': ('/', ('name', 'x'), ('value', 10))}],
    ['x cross y',
     {'exp': ('*', ('name', 'x'), ('name', 'y'))}],
]

func_ips = [
    ['declare a function func1',
     {'request': 'declare', 'construct': 'function',
      'name': 'func1'}],
    ['declare a function abc of type int',
     {'request': 'declare', 'construct': 'function',
      'name': 'abc', 'type': 'int'}],
    ['define a function of return type int func1',
     {'request': 'declare', 'construct': 'function',
      'name': 'func1', 'type': 'int'}],
    ['define a function with name fun',
     {'request': 'declare', 'construct': 'function',
      'name': 'fun'}],
    ['create a function having return type int called func1',
     {'request': 'declare', 'construct': 'function',
      'name': 'func1', 'type': 'int'}],
    ['define a function called func of return type double',
     {'request': 'declare', 'construct': 'function',
      'name': 'func', 'type': 'double'}],
    ['declare a function called func with return value of type int',
     {'request': 'declare', 'construct': 'function',
      'name': 'func', 'type': 'int'}],
    ['create an integer type function func1',
     {'request': 'declare', 'construct': 'function',
      'name': 'func1', 'type': 'int'}],
    # 'declare a function func1 of type int and\
    #  accepts parameter x of type double',
    # 'define a function abc of return type int and\
    #  parameters of type int and char',
    # 'create a function hey that returns an integer\
    #  and accepts two integer arguments called a and b',
    # 'create an integer type function func having a parameter x of type int,\
    #  a parameter y of type char and a z of type int',
    # 'declare a function called fun of type float\
    #  that accepts integer argument x and character argument y',
    # 'create a function func having parameters x of type int, \
    # parameter y of type char and z of type int and return type int',
    # 'create a function having name func1 return type int and\
    #  parameters integer type x charater type y and integer type z',
]

func_param_ips = [
    ['add parameter x of type double',
     {'request': 'add', 'construct': 'parameter',
      'name': 'x', 'type': 'double'}],
    ['add parameter called x of type double',
     {'request': 'add', 'construct': 'parameter',
      'name': 'x', 'type': 'double'}],
    ['add parameter integer x',
     {'request': 'add', 'construct': 'parameter',
      'name': 'x', 'type': 'int'}],
    ['add a parameter of type double with name x',
     {'request': 'add', 'construct': 'parameter',
      'name': 'x', 'type': 'double'}],
    ['add a parameter of integer type called x',
     {'request': 'add', 'construct': 'parameter',
      'name': 'x', 'type': 'int'}],
    ['add an integer type parameter called max',
     {'request': 'add', 'construct': 'parameter',
      'name': 'max', 'type': 'int'}],
    ['add a parameter x of type double at position 3',
     {'request': 'add', 'construct': 'parameter',
      'name': 'x', 'type': 'double', 'position': 3}],
    ['add parameter x in position 3 of type double',
     {'request': 'add', 'construct': 'parameter',
      'name': 'x', 'type': 'double', 'position': 3}],
    ['add a parameter at position 2 called x of type double',
     {'request': 'add', 'construct': 'parameter',
      'name': 'x', 'type': 'double', 'position': 2}],
    ['add a parameter of integer type called x in position 4',
     {'request': 'add', 'construct': 'parameter',
      'name': 'x', 'type': 'int', 'position': 4}],
    ['add a parameter of integer type at position 1 called x',
     {'request': 'add', 'construct': 'parameter',
      'name': 'x', 'type': 'int', 'position': 1}],
    ['add a parameter at position 7 of integer type called x',
     {'request': 'add', 'construct': 'parameter',
      'name': 'x', 'type': 'int', 'position': 7}],
    ['add an integer type parameter with name max in position 4',
     {'request': 'add', 'construct': 'parameter',
      'name': 'max', 'type': 'int', 'position': 4}],
    ['add an integer type parameter at the third position called max',
     {'request': 'add', 'construct': 'parameter',
      'name': 'max', 'type': 'int', 'position': 3}],
    ['add a parameter at fourth position called x of type double',
     {'request': 'add', 'construct': 'parameter',
      'name': 'x', 'type': 'double', 'position': 4}],
    ['add a parameter of integer type called x in the fifth position',
     {'request': 'add', 'construct': 'parameter',
      'name': 'x', 'type': 'int', 'position': 5}],
    ['add a parameter in the second position of integer type called x',
     {'request': 'add', 'construct': 'parameter',
      'name': 'x', 'type': 'int', 'position': 2}],
    ['add parameter called x of type integer pointer',
     {'request': 'add', 'construct': 'parameter',
      'name': 'x', 'type': 'int', 'modifier': 'pointer'}],
]

func_call_ips = [
    ['call function called max',
     {'request': 'declare', 'construct': 'func_call', 'name': 'max'}],
    ['call function printf',
     {'request': 'declare', 'construct': 'func_call', 'name': 'printf'}],
    ['invoke the function ceil with parameter x',
     {'request': 'declare', 'construct': 'func_call',
      'name': 'ceil', 'parameters': (('name', 'x'), )}],
    ['call function max with arguments x and y',
     {'request': 'declare', 'construct': 'func_call',
      'name': 'max', 'parameters': (('name', 'x'), ('name', 'y'))}],
    ['call function called reverse with argument x',
     {'request': 'declare', 'construct': 'func_call',
      'name': 'reverse', 'parameters': (('name', 'x'), )}],
    ['execute function called toLowerCase with parameter x',
     {'request': 'declare', 'construct': 'func_call',
      'name': 'toLowerCase', 'parameters': (('name', 'x'), )}],
    ['call the function max and pass it arguments 2 and 3',
     {'request': 'declare', 'construct': 'func_call',
      'name': 'max', 'parameters': (('value', 2), ('value', 3))}],
    # 'call function func and pass it as arguments the variables x and y', not handling(..args variable x and func y)
    ['invoke function fun and pass parameters x and y',
     {'request': 'declare', 'construct': 'func_call',
      'name': 'fun', 'parameters': (('name', 'x'), ('name', 'y'))}],
    ['execute function func and give arguments g and h',
     {'request': 'declare', 'construct': 'func_call',
      'name': 'func', 'parameters': (('name', 'g'), ('name', 'h'))}],
    ['invoke the function called printf and pass it parameters 6 and y',
     {'request': 'declare', 'construct': 'func_call',
      'name': 'printf', 'parameters': (('str', '6'), ('name', 'y'))}],
    ['invoke function max having parameters 2 and 10 and 5',
     {'request': 'declare', 'construct': 'func_call',
      'name': 'max', 'parameters': (('value', 2), ('value', 10), ('value', 5))}],
    ['call the function func1 and pass it arguments 2 3 and 4',
     {'request': 'declare', 'construct': 'func_call',
      'name': 'func1',
      'parameters': (('value', 2), ('value', 3), ('value', 4))}],
]

f_c_printf_ips = [
    ['call printf with string Hello World',
     {'request': 'declare', 'construct': 'func_call',
      'name': 'printf', 'parameters': (('str', 'Hello World'), )}],
    ['call function printf with string type argument Hey',
     {'request': 'declare', 'construct': 'func_call',
      'name': 'printf', 'parameters': (('str', 'Hey'), )}],
    ['invoke the function printf with parameter Hello',
     {'request': 'declare', 'construct': 'func_call',
      'name': 'printf', 'parameters': (('str', 'Hello'), )}],
    ['call function printf with argument of string type Hello World',
     {'request': 'declare', 'construct': 'func_call',
      'name': 'printf', 'parameters': (('str', 'Hello World'), )}],
    ['call function called printf with string Hey',
     {'request': 'declare', 'construct': 'func_call',
      'name': 'printf', 'parameters': (('str', 'Hey'), )}],
    ['execute function called printf with parameter of type string Demoing Printf',
     {'request': 'declare', 'construct': 'func_call',
      'name': 'printf', 'parameters': (('str', 'Demoing Printf'), )}],
    ['call the function printf and pass it string argument Hello World',
     {'request': 'declare', 'construct': 'func_call',
      'name': 'printf', 'parameters': (('str', 'Hello World'), )}],
    ['invoke function printf and pass string type parameter Test',
     {'request': 'declare', 'construct': 'func_call',
      'name': 'printf', 'parameters': (('str', 'Test'), )}],
    ['execute function printf and give argument of type string Hi',
     {'request': 'declare', 'construct': 'func_call',
      'name': 'printf', 'parameters': (('str', 'Hi'), )}],
    ['call printf with parameters Hi and variable name',
     {'request': 'declare', 'construct': 'func_call',
      'name': 'printf', 'parameters': (('str', 'Hi'), ('name', 'name'))}],
    ['call the function printf with string Total Votes and integer parameter count',
     {'request': 'declare', 'construct': 'func_call',
      'name': 'printf', 'parameters': (('str', 'Total Votes'), ('name', 'count'))}],
    ['call printf with string Value of variable is and parameter x',
     {'request': 'declare', 'construct': 'func_call',
      'name': 'printf', 'parameters': (('str', 'Value of variable is'), ('name', 'x'))}],
    ['call printf with string Greetings and parameter user',
     {'request': 'declare', 'construct': 'func_call',
      'name': 'printf', 'parameters': (('str', 'Greetings'), ('name', 'user'))}],
    ['invoke function printf with string Greetings and parameter person',
     {'request': 'declare', 'construct': 'func_call',
      'name': 'printf', 'parameters': (('str', 'Greetings'), ('name', 'person'))}],
    ['invoke function printf with parameters of type string Value of Max and parameter max',
     {'request': 'declare', 'construct': 'func_call',
      'name': 'printf', 'parameters': (('str', 'Value of Max'), ('name', 'max'))}],
    ['call function printf with string Today is and integer parameter date',
     {'request': 'declare', 'construct': 'func_call',
      'name': 'printf', 'parameters': (('str', 'Today is'), ('name', 'date'))}],
    ['invoke function printf with parameters of type string Value of Max and float result',
     {'request': 'declare', 'construct': 'func_call',
      'name': 'printf', 'parameters': (('str', 'Value of Max'), ('name', 'result'))}],
    # ['return value of function printf with argument Hello World',
    #  {'request': 'declare', 'construct': 'func_call',
    #   'name': 'printf', 'parameters': (('str', 'Hello World'), )}],
]

f_c_scanf_ips = [
    ['call scanf with parameters mod d and address of x',
     {'request': 'declare', 'construct': 'func_call',
      'name': 'scanf', 'parameters': (('str', '%d'), ('name', '&x'))}],
    ['call scanf and pass it string mod s and parameter text',
     {'request': 'declare', 'construct': 'func_call',
      'name': 'scanf', 'parameters': (('str', '%s'), ('name', 'text'))}],
    ['invoke scanf and pass parameter mod s and parameter text',
     {'request': 'declare', 'construct': 'func_call',
      'name': 'scanf', 'parameters': (('str', '%s'), ('name', 'text'))}],
    ['call scanf with string Enter Name: mod s and parameter user',
     {'request': 'declare', 'construct': 'func_call',
      'name': 'scanf', 'parameters': (('str', 'Enter Name: %s'), ('name', 'user'))}],
    ['call scanf and pass it string Enter Value: modulus d and parameter address of limit',
     {'request': 'declare', 'construct': 'func_call',
      'name': 'scanf', 'parameters': (('str', 'Enter Value: %d'), ('name', '&limit'))}],
    ['call scanf with parameters mod d and an integer parameter address of x',
     {'request': 'declare', 'construct': 'func_call',
      'name': 'scanf', 'parameters': (('str', '%d'), ('name', '&x'))}],
    ['invoke function scanf with string modulus s and parameter person',
     {'request': 'declare', 'construct': 'func_call',
      'name': 'scanf', 'parameters': (('str', '%s'), ('name', 'person'))}],
    ['invoke function scanf with parameters of type string Value of Max mod d and address of max',
     {'request': 'declare', 'construct': 'func_call',
      'name': 'scanf', 'parameters': (('str', 'Value of Max %d'), ('name', '&max'))}],
    ['call function scanf with string Today is modulus d and parameter address of date',
     {'request': 'declare', 'construct': 'func_call',
      'name': 'scanf', 'parameters': (('str', 'Today is %d'), ('name', '&date'))}],
    ['invoke scanf with parameters string Value of Max is mod f and address of result',
     {'request': 'declare', 'construct': 'func_call',
      'name': 'scanf', 'parameters': (('str', 'Value of Max is %f'), ('name', '&result'))}],
    ['call function called scanf with parameters Today is modulus d and address of date',
     {'request': 'declare', 'construct': 'func_call',
      'name': 'scanf', 'parameters': (('str', 'Today is %d'), ('name', '&date'))}],    
]

init_ips = [
    ['initialize variable x',
     {'request': 'declare', 'construct': 'assignment',
      'lhs': ('name', 'x')}],
    ['initialize variable x equal to 4',
     {'request': 'declare', 'construct': 'assignment',
      'lhs': ('name', 'x'), 'rhs': ('value', 4)}],
    ['initialize x to 4',
     {'request': 'declare', 'construct': 'assignment',
      'lhs': ('name', 'x'), 'rhs': ('value', 4)}],
    ['initialize x to y',
     {'request': 'declare', 'construct': 'assignment',
      'lhs': ('name', 'x'), 'rhs': ('name', 'y')}],
    ['set x equals 20',
     {'request': 'declare', 'construct': 'assignment',
      'lhs': ('name', 'x'), 'rhs': ('value', 20)}],
    ['set x to y',
     {'request': 'declare', 'construct': 'assignment',
      'lhs': ('name', 'x'), 'rhs': ('name', 'y')}],
    ['make y equals 20',
     {'request': 'declare', 'construct': 'assignment',
      'lhs': ('name', 'y'), 'rhs': ('value', 20)}],
    ['make y equal to 20',
     {'request': 'declare', 'construct': 'assignment',
      'lhs': ('name', 'y'), 'rhs': ('value', 20)}],
    ['initialize value of x to 15',
     {'request': 'declare', 'construct': 'assignment',
      'lhs': ('name', 'x'), 'rhs': ('value', 15)}],
    ['set value of y to 20',
     {'request': 'declare', 'construct': 'assignment',
      'lhs': ('name', 'y'), 'rhs': ('value', 20)}],
        ['make the value of variable x to variable y',
        {'request': 'declare', 'construct': 'assignment',
        'lhs': ('name', 'x'), 'rhs': ('name', 'y')}],

    # pointer inputs
    ['initialize ptr to x',
     {'request': 'declare', 'construct': 'assignment',
      'lhs': ('name', 'ptr'), 'rhs': ('name', 'x')}],
    ['initialize ptr to address of x',
     {'request': 'declare', 'construct': 'assignment',
      'lhs': ('name', 'ptr'), 'rhs': ('&', ('name', 'x'))}],
    ['set pointer x to pointer y',
     {'request': 'declare', 'construct': 'assignment',
      'lhs': ('name', 'x'), 'rhs': ('name', 'y')}],
    ['set ptr to address of variable x',
     {'request': 'declare', 'construct': 'assignment',
      'lhs': ('name', 'ptr'), 'rhs': ('&', ('name', 'x'))}],
    ['initialize pointer x to pointer y',
     {'request': 'declare', 'construct': 'assignment',
      'lhs': ('name', 'x'), 'rhs': ('name', 'y')}],
    ['set pointer x to address of array y',
     {'request': 'declare', 'construct': 'assignment',
      'lhs': ('name', 'x'), 'rhs': ('&', ('name', 'y'))}],

    # array index init
    ['set value of array a at index j equal to value of\
      array a at index j plus 1',
     {'request': 'declare', 'construct': 'assignment',
      'lhs': ('[]', ('name', 'a'), ('name', 'j')),
      'rhs': ('+', ('[]', ('name', 'a'), ('name', 'j')), ('value', 1))}],
    ['set a of i to 10',
     {'request': 'declare', 'construct': 'assignment',
      'lhs': ('[]', ('name', 'a'), ('name', 'i')),
      'rhs': ('value', 10)}]
]

loop_ips = [
    ['create a for loop',
     {'request': 'declare', 'construct': 'loop',
      'type': 'for'}],
    ['define a while loop',
     {'request': 'declare', 'construct': 'loop',
      'type': 'while'}],
    ['create a while loop',
     {'request': 'declare', 'construct': 'loop',
      'type': 'while'}],
    ['declare a while loop',
     {'request': 'declare', 'construct': 'loop',
      'type': 'while'}],
    ['run a for loop',
     {'request': 'declare', 'construct': 'loop',
      'type': 'for'}],
    ['define a do while loop',
     {'request': 'declare', 'construct': 'loop',
      'type': 'do while'}],
    ['declare a loop',
     {'request': 'declare', 'construct': 'loop'}],
]

loop_init_ips = [
    ['add loop variable i of type char',
     {'request': 'add', 'construct': 'init',
      'name': 'i', 'type': 'char'}],
    ['add counter i of type int',
     {'request': 'add', 'construct': 'init',
      'name': 'i', 'type': 'int'}],
    ['add integer type initializer called i',
     {'request': 'add', 'construct': 'init',
      'name': 'i', 'type': 'int'}],
    ['add control variable called i having type integer',
     {'request': 'add', 'construct': 'init',
      'name': 'i', 'type': 'int'}],
    ['add variable of integer type called i',
     {'request': 'add', 'construct': 'init',
      'name': 'i', 'type': 'int'}],
    ['add initializer of type int called i',
     {'request': 'add', 'construct': 'init',
      'name': 'i', 'type': 'int'}],
    ['add integer type variable i equals 0',
     {'request': 'add', 'construct': 'init',
      'name': 'i', 'type': 'int', 'value': ('value', 0)}],
    ['add loop variable i equals 0',
     {'request': 'add', 'construct': 'init',
      'name': 'i', 'value': ('value', 0)}],
    ['add control variable i of type integer initialized to 0',
     {'request': 'add', 'construct': 'init',
      'name': 'i', 'type': 'int', 'value': ('value', 0)}],
    ['add integer type loop variable i set to 0',
     {'request': 'add', 'construct': 'init',
      'name': 'i', 'type': 'int', 'value': ('value', 0)}],
    ['add control variable int i equals 0',
     {'request': 'add', 'construct': 'init',
      'name': 'i', 'type': 'int', 'value': ('value', 0)}],
    ['add variable i equals 0 of type integer',
     {'request': 'add', 'construct': 'init',
      'name': 'i', 'type': 'int', 'value': ('value', 0)}],
    ['add loop variable i of integer type set to 0',
     {'request': 'add', 'construct': 'init',
      'name': 'i', 'type': 'int', 'value': ('value', 0)}],
]

loop_cond_ips = [
    ['add condition x less than 10',
     {'request': 'add', 'construct': 'condition',
      'cond': ('<', ('name', 'x'), ('value', 10))}],
    ['add a condition program count greater than 20',
     {'request': 'add', 'construct': 'condition',
      'cond': ('>', ('name', 'program_count'), ('value', 20))}],
    ['add the condition a greater than 2',
     {'request': 'add', 'construct': 'condition',
      'cond': ('>', ('name', 'a'), ('value', 2))}],
    ['add conditions a is greater than 5 and a is less than 10',
     {'request': 'add', 'construct': 'condition',
      'cond': ('&&',
               ('>', ('name', 'a'), ('value', 5)), 
               ('<', ('name', 'a'), ('value', 10)))}],
    ['add loop condition a is equal to 10',
     {'request': 'add', 'construct': 'condition',
      'cond': ('==', ('name', 'a'), ('value', 10))}],
    ['add the condition a is not equal to 10',
     {'request': 'add', 'construct': 'condition',
      'cond': ('!=', ('name', 'a'), ('value', 10))}],
    ['add condition x equals 10',
     {'request': 'add', 'construct': 'condition',
      'cond': ('==', ('name', 'x'), ('value', 10))}],
    ['add a condition a equal to 10',
     {'request': 'add', 'construct': 'condition',
      'cond': ('==', ('name', 'a'), ('value', 10))}],
    ['add loop condition a not equal to 10',
     {'request': 'add', 'construct': 'condition',
      'cond': ('!=', ('name', 'a'), ('value', 10))}],
    ['add a loop condition x not greater than 5',
     {'request': 'add', 'construct': 'condition',
      'cond': ('<=', ('name', 'x'), ('value', 5))}],
]

loop_update_ips = [
    ['add an update plus plus a',
     {'request': 'add', 'construct': 'update',
      'updates': ('++', ('name', 'a'))}],
    ['add a loop update x plus plus',
     {'request': 'add', 'construct': 'update',
      'updates': ('p++', ('name', 'x'))}],
    ['add loop step y plus plus',
     {'request': 'add', 'construct': 'update',
      'updates': ('p++', ('name', 'y'))}],
    ['add a loop modifier x minus minus',
     {'request': 'add', 'construct': 'update',
      'updates': ('p--', ('name', 'x'))}],
    ['add increment a plus plus',
     {'request': 'add', 'construct': 'update',
      'updates': ('p++', ('name', 'a'))}],
    ['add update statement x plus plus',
     {'request': 'add', 'construct': 'update',
      'updates': ('p++', ('name', 'x'))}],
    ['add decrement minus minus z',
     {'request': 'add', 'construct': 'update',
      'updates': ('--', ('name', 'z'))}],
    # ['add modifiers ++x and --y',
    #  {'request': 'add', 'construct': 'update',
    #   'updates': (('++', ('name', 'y')), ('--', ('name', 'y')))}],
    # 'add loop updates a++ and --b',
    # 'add update statements x++ and y--',
    # 'add loop modifiers x++ --y and z--',
    # 'add loop steps ++a --b and c--',
    # 'add updates x++ ++z and y--',
]

nav_ips = [
    ['goto line number 10',
     {'request': 'navigate', 'construct': 'line',
      'value': 10}],
    ['goto line 10',
     {'request': 'navigate', 'construct': 'line',
      'value': 10}],
    ['goto the end of line',
     {'request': 'navigate', 'construct': 'line',
      'direction': 'end'}],
    ['goto the start of line',
     {'request': 'navigate', 'construct': 'line',
      'direction': 'start'}],
    ['go to the next line',
     {'request': 'navigate', 'construct': 'line',
      'direction': 'next'}],
    ['go a line up',
     {'request': 'navigate', 'construct': 'line',
      'direction': 'up'}],
    ['go 8 lines up',
     {'request': 'navigate', 'construct': 'line',
      'direction': 'up', 'value': 8}],
    ['go down one line',
     {'request': 'navigate', 'construct': 'line',
      'direction': 'down', 'value': 1}],
    ['go to the previous line',
     {'request': 'navigate', 'construct': 'line',
      'direction': 'back'}],
    ['go three positions back',
     {'request': 'navigate', 'construct': 'character',
      'direction': 'back', 'value': 3}],
    ['go back 2 places',
     {'request': 'navigate', 'construct': 'character',
      'direction': 'back', 'value': 2}],
    ['go four places front',
     {'request': 'navigate', 'construct': 'character',
      'direction': 'next', 'value': 4}],
    # 'goto inner loop',
    # 'go to the inner if statement',
    # 'goto function func',
]

edit_ips = [
    ['undo',
     {'request': 'edit', 'construct': 'undo'}],
    ['redo',
     {'request': 'edit', 'construct': 'redo'}],
    ['paste',
     {'request': 'edit', 'construct': 'paste'}],
    ['copy current line',
     {'request': 'edit', 'construct': 'copy',
      'direction': 'current', 'e_type': 'line'}],
    ['copy line 3',
     {'request': 'edit', 'construct': 'copy',
      'e_type': 'line', 'value': (3,)}],
    ['copy lines 3 to 5',
     {'request': 'edit', 'construct': 'copy',
      'e_type': 'line', 'value': (3, 5)}],
    ['copy lines 3 and 5',
     {'request': 'edit', 'construct': 'copy',
      'e_type': 'line', 'value': (3, 5)}],
    ['copy lines 3 4 and 5',
     {'request': 'edit', 'construct': 'copy',
      'e_type': 'line', 'value': (3, 4, 5)}],
    # Delete type commands
    ['delete function max',
     {'request': 'edit', 'construct': 'delete',
      'e_type': 'function', 'name': 'max'}],
    ['delete variable x',
     {'request': 'edit', 'construct': 'delete',
      'e_type': 'variable', 'name': 'x'}],
    ['delete current line',
     {'request': 'edit', 'construct': 'delete',
      'direction': 'current', 'e_type': 'line'}],
    ['delete next 2 lines',
     {'request': 'edit', 'construct': 'delete',
      'direction': 'next', 'value': (2,), 'e_type': 'line'}],
    ['delete lines 3 to 10',
     {'request': 'edit', 'construct': 'delete',
      'e_type': 'line', 'value': (3, 10)}],
    ['delete lines 3 4 6 7 and 10',
     {'request': 'edit', 'construct': 'delete',
      'e_type': 'line', 'value': (3, 4, 6, 7, 10)}],
    # Update type commands
    ['change x to 10',
     {'request': 'edit', 'construct': 'change',
      'lhs': ('name', 'x'), 'rhs': ('value', 10)}],
    ['change variable x to variable y',
     {'request': 'edit', 'construct': 'change',
      'lhs': ('name', 'x'), 'rhs': ('name', 'y')}],
    ['change x to 15',
     {'request': 'edit', 'construct': 'change',
      'lhs': ('name', 'x'), 'rhs': ('value', 15)}],
    ['update value of y to 20',
     {'request': 'edit', 'construct': 'change',
      'lhs': ('name', 'y'), 'rhs': ('value', 20)}],
    ['update the value of variable x to variable y',
     {'request': 'edit', 'construct': 'change',
      'lhs': ('name', 'x'), 'rhs': ('name', 'y')}],
]

additional_ips = [
    # 'exit',
    # 'exit block',
    # 'leave current block',
    'start dictation',
    'stop dictation',
]

pack_ips = [
    ['include the package stdio',
     {'request': 'include', 'construct': 'package',
      'name': 'stdio', 'type': 'lib'}],
    ['include a library package called stdio',
     {'request': 'include', 'construct': 'package',
      'name': 'stdio', 'type': 'lib'}],
    ['include the library stdlib',
     {'request': 'include', 'construct': 'package',
      'name': 'stdlib', 'type': 'lib'}],
    ['include package string',
     {'request': 'include', 'construct': 'package',
      'name': 'string', 'type': 'lib'}],
    ['include package with name stdlib',
     {'request': 'include', 'construct': 'package',
      'name': 'stdlib', 'type': 'lib'}],
    ['include library package called stdarg',
     {'request': 'include', 'construct': 'package',
      'name': 'stdarg', 'type': 'lib'}],
    ['include package with name MyLib',
     {'request': 'include', 'construct': 'package',
      'name': 'MyLib', 'type': 'own'}],
    ['include package called SupportFunc',
     {'request': 'include', 'construct': 'package',
      'name': 'SupportFunc', 'type': 'own'}],
    ['include package linked list',
     {'request': 'include', 'construct': 'package',
      'name': 'linked_list', 'type': 'own'}],
]

ptr_ips = [
    ['declare integer pointer ptr',
     {'request': 'declare', 'construct': 'variable',
      'name': 'ptr', 'type': 'int', 'modifier': 'pointer'}],
    ['declare pointer x of type integer',
     {'request': 'declare', 'construct': 'variable',
      'name': 'x', 'type': 'int', 'modifier': 'pointer'}],
    ['declare integer pointer called x',
     {'request': 'declare', 'construct': 'variable',
      'name': 'x', 'type': 'int', 'modifier': 'pointer'}],
    ['declare an integer pointer ptr',
     {'request': 'declare', 'construct': 'variable',
      'name': 'ptr', 'type': 'int', 'modifier': 'pointer'}],
    ['declare a pointer y of type int',
     {'request': 'declare', 'construct': 'variable',
      'name': 'y', 'type': 'int', 'modifier': 'pointer'}],
    ['declare a double pointer called z',
     {'request': 'declare', 'construct': 'variable',
      'name': 'z', 'type': 'double', 'modifier': 'pointer'}],
    ['declare a pointer called ptr idx of type double',
     {'request': 'declare', 'construct': 'variable',
      'name': 'ptr_idx', 'type': 'double', 'modifier': 'pointer'}],
    ['declare a pointer called x with type float',
     {'request': 'declare', 'construct': 'variable',
      'name': 'x', 'type': 'float', 'modifier': 'pointer'}],
    ['declare an integer pointer with name y',
     {'request': 'declare', 'construct': 'variable',
      'name': 'y', 'type': 'int', 'modifier': 'pointer'}],
    ['declare pointer x',
     {'request': 'declare', 'construct': 'variable',
      'name': 'x', 'modifier': 'pointer'}],
    ['declare a pointer of type integer x',
     {'request': 'declare', 'construct': 'variable',
      'name': 'x', 'type': 'int', 'modifier': 'pointer'}],
    ['declare integer type pointer ptr',
     {'request': 'declare', 'construct': 'variable',
      'name': 'ptr', 'type': 'int', 'modifier': 'pointer'}],
    ['declare a pointer count with type integer',
     {'request': 'declare', 'construct': 'variable',
      'name': 'count', 'type': 'int', 'modifier': 'pointer'}],
    ['declare variable called ptr of type integer pointer',
     {'request': 'declare', 'construct': 'variable',
      'name': 'ptr', 'type': 'int', 'modifier': 'pointer'}],
    ['declare a pointer to an integer variable called p',
     {'request': 'declare', 'construct': 'variable',
      'name': 'p', 'type': 'int', 'modifier': 'pointer'}],
    ['declare a pointer p of type integer',
     {'request': 'declare', 'construct': 'variable',
      'name': 'p', 'type': 'int', 'modifier': 'pointer'}],
    ['declare a pointer variable p of integer type',
     {'request': 'declare', 'construct': 'variable',
      'name': 'p', 'type': 'int', 'modifier': 'pointer'}],
    ['declare a pointer p to integer',
     {'request': 'declare', 'construct': 'variable',
      'name': 'p', 'type': 'int', 'modifier': 'pointer'}],
    ['declare a variable that points to integer called p',
     {'request': 'declare', 'construct': 'variable',
      'name': 'p', 'type': 'int', 'modifier': 'pointer'}],
    ['declare a variable pointing to integer',
     {'request': 'declare', 'construct': 'variable',
      'type': 'int', 'modifier': 'pointer'}],
    ['declare a pointer p',
     {'request': 'declare', 'construct': 'variable',
      'name': 'p', 'modifier': 'pointer'}],
    ['declare pointer ptr of integer',
     {'request': 'declare', 'construct': 'variable',
      'name': 'ptr', 'type': 'int', 'modifier': 'pointer'}],
]

return_stmt_ips = [
    ['return abc',
     {'request': 'declare', 'construct': 'return',
      'value': ('name', 'abc')}],
    ['return the value of max',
     {'request': 'declare', 'construct': 'return',
      'value': ('name', 'max')}],
    ['return the variable min',
     {'request': 'declare', 'construct': 'return',
      'value': ('name', 'min'), 'type': 'variable'}],
    ['return the value of variable sum',
     {'request': 'declare', 'construct': 'return',
      'value': ('name', 'sum'), 'type': 'variable'}],
    ['return the variable called abc',
     {'request': 'declare', 'construct': 'return',
      'value': ('name', 'abc'), 'type': 'variable'}],
    ['return the variable with name min',
     {'request': 'declare', 'construct': 'return',
      'value': ('name', 'min'), 'type': 'variable'}],
    ['return the value 100',
     {'request': 'declare', 'construct': 'return',
      'value': ('value', 100)}],
    ['return 0',
     {'request': 'declare', 'construct': 'return',
      'value': ('value', 0)}],
    ['return x',
     {'request': 'declare', 'construct': 'return',
      'value': ('name', 'x')}],
    ['return null',
     {'request': 'declare', 'construct': 'return',
      'value': ('value', 'null')}],
    ['return the array x',
     {'request': 'declare', 'construct': 'return',
      'value': ('name', 'x'), 'type': 'array'}],
    ['return the array called arr',
     {'request': 'declare', 'construct': 'return',
      'value': ('name', 'arr'), 'type': 'array'}],
    ['return the function max',
     {'request': 'declare', 'construct': 'return',
      'value': ('name', 'max'), 'type': 'function'}],
    # ['return the string hello world',
    #  {'request': 'declare', 'construct': 'return',
    #   'value': ('value', 'hello world')}],
    ['return the value of function max',
     {'request': 'declare', 'construct': 'return',
      'value': ('name', 'max'), 'type': 'function'}],
]

struct_ips = [
    'declare a structure called Employee',
    'declare a structure with name books',
    'create a structure with tag Address',
    'declare a structure with structure tag employee ',
    'create a structure called Books with variable book1',
    'define a structure with name employee having struct variable emp1',
    'create a structure called Books with variables book1 and book2',
    'define a structure with tag Address and struct variables addr1 and addr2',
    'define a structure employee with struct variables emp1 emp2 emp3',
    'create a structure called Address and struct variables addr1 addr2 and addr3',
    'create a structure with struct tag Books and variables book1 book2 book3',
    'define a structure with tag Employee and struct variables x and y and z',
    'declare a structure with structure tag address and variables var1 var2 and var3',     #erroneous (due to length)
]

var_ips = [
    ['declare variable x of type integer set to 0',
     {'request': 'declare', 'construct': 'variable',
      'name': 'x', 'type': 'int', 'value': ('value', 0)}],
    ['declare a variable of type integer called x set to zero',
     {'request': 'declare', 'construct': 'variable',
      'name': 'x', 'type': 'int', 'value': ('value', 0)}],
    ['declare integer variable x',
     {'request': 'declare', 'construct': 'variable',
      'name': 'x', 'type': 'int'}],
    ['declare variable x type integer',
     {'request': 'declare', 'construct': 'variable',
      'name': 'x', 'type': 'int'}],
    ['declare integer variable called x',
     {'request': 'declare', 'construct': 'variable',
      'name': 'x', 'type': 'int'}],
    ['declare an integer variable x',
     {'request': 'declare', 'construct': 'variable',
      'name': 'x', 'type': 'int'}],
    ['declare a variable y of type int',
     {'request': 'declare', 'construct': 'variable',
      'name': 'y', 'type': 'int'}],
    ['declare an int variable x',
     {'request': 'declare', 'construct': 'variable',
      'name': 'x', 'type': 'int'}],
    ['declare a double variable called z',
     {'request': 'declare', 'construct': 'variable',
      'name': 'z', 'type': 'double'}],
    ['declare a variable called prog count of type double',
     {'request': 'declare', 'construct': 'variable',
      'name': 'prog_count', 'type': 'double'}],
    ['declare a variable x with type integer',
     {'request': 'declare', 'construct': 'variable',
      'name': 'x', 'type': 'int'}],
    ['declare a variable called x with type float',
     {'request': 'declare', 'construct': 'variable',
      'name': 'x', 'type': 'float'}],
    ['declare a variable called prog count of type int',
     {'request': 'declare', 'construct': 'variable',
      'name': 'prog_count', 'type': 'int'}],
    ['declare an integer variable with name y',
     {'request': 'declare', 'construct': 'variable',
      'name': 'y', 'type': 'int'}],
    ['declare variable x',
     {'request': 'declare', 'construct': 'variable',
      'name': 'x'}],
    ['declare a variable of type integer x',
     {'request': 'declare', 'construct': 'variable',
      'name': 'x', 'type': 'int'}],
    ['declare a variable of integer type called program counter',
     {'request': 'declare', 'construct': 'variable',
      'name': 'program_counter', 'type': 'int'}],
    ['declare integer type variable program counter',
     {'request': 'declare', 'construct': 'variable',
      'name': 'program_counter', 'type': 'int'}],
    ['declare an integer x',
     {'request': 'declare', 'construct': 'variable',
      'name': 'x', 'type': 'int'}],
    ['declare a variable count with type of integer',
     {'request': 'declare', 'construct': 'variable',
      'name': 'count', 'type': 'int'}],
    ['declare a long integer program count',
     {'request': 'declare', 'construct': 'variable',
      'name': 'program_count', 'type': 'long int'}]
]

if_ips = [
    ['declare an if statement',
     {'request': 'declare', 'construct': 'if'}],
    ['declare a conditional statement',
     {'request': 'declare', 'construct': 'if'}],
    ['declare if',
     {'request': 'declare', 'construct': 'if'}],
    ['declare if else',
     {'request': 'declare', 'construct': 'if_else'}],
    ['declare if else statement',
     {'request': 'declare', 'construct': 'if_else'}],

    # adding conditions and else if
    ['add else if block',
     {'request': 'add', 'construct': 'else_if'}],
    ['add else if',
     {'request': 'add', 'construct': 'else_if'}],
    ['add condition x greater than 2',
     {'request': 'add', 'construct': 'condition',
      'cond': ('>', ('name', 'x'), ('value', 2))}],
    ['add condition x less than y and z greater than x',
     {'request': 'add', 'construct': 'condition',
      'cond': ('&&',
               ('<', ('name', 'x'), ('name', 'y')),
               ('>', ('name', 'z'), ('name', 'x')))}],
    ['add a condition x less than 10',
     {'request': 'add', 'construct': 'condition',
      'cond': ('<', ('name', 'x'), ('value', 10))}],
    ['add else',
     {'request': 'add', 'construct': 'else'}],
]

array_index_ips = [
    ['x of i',
     {'exp': ('[]', ('name', 'x'), ('name', 'i'))}],
    ['x sub i',
     {'exp': ('[]', ('name', 'x'), ('name', 'i'))}],
    ['element at i of a',
     {'exp': ('[]', ('name', 'a'), ('name', 'i'))}],
    ['element at index i of a',
     {'exp': ('[]', ('name', 'a'), ('name', 'i'))}],
    ['element of y at index i',
     {'exp': ('[]', ('name', 'y'), ('name', 'i'))}],
    ['element of a at i',
     {'exp': ('[]', ('name', 'a'), ('name', 'i'))}],
    ['element of a at position i',
     {'exp': ('[]', ('name', 'a'), ('name', 'i'))}],
    ['element j of array a',
     {'exp': ('[]', ('name', 'a'), ('name', 'j'))}],
    ['value of array y at index i',
     {'exp': ('[]', ('name', 'y'), ('name', 'i'))}],
]

'''
loop_ips = [
    'create a for loop with loop variable i',
    'create a for loop with counter i of type int',
    'define a loop having control variable i',
    'create a for loop with counter of type int called i',
    'declare a while loop with counter i of type int',
    'run a for loop with loop counter i of type int',
    'make a while loop with condition i less 10',
    'declare a while loop that runs until i equals 0',
    'run a for loop that goes from i equal to zero to i equal to 10',
    'define a loop with initializer i going from 0 to 10 in steps of 2',
]
'''
