...
def get_action(transcripts, filename, line):
    codes = []
    for transcript in transcripts:
        uniq_sems = get_uniq_sems(transcript)
        if isinstance(uniq_sems, dict):
            codes.append(uniq_sems)
            continue
        code = generate_code(uniq_sems[:5], filename, line)
        codes.append({'input': transcript, 'children': tuple(code)})
...
if __name__ == "__main__":
    ips = [
        ...
        var_ips, if_ips, array_index_ips, return_stmt_ips         # [16 - 19]
    ]
